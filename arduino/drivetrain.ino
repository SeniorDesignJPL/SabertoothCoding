...
USBSabertoothSerial C(SabertoothSerial2x5); // Use SWSerial as the serial port.
USBSabertooth USB_ST = USBSabertooth(C, 130); // Use SWSerial as the serial port.
	...
	for (int i = 0; i < 6; i++) {
		if (prev_speeds[i] != wheel_speeds[i]) {
			switch (i) {
				...
				case 2: ST_1.motor(1, wheel_speeds[i]);
				case 3: ST_1.motor(2, wheel_speeds[i]);
				case 4: ST_2.motor(1, wheel_speeds[i]);
				case 5: ST_2.motor(2, wheel_speeds[i]);
			}
